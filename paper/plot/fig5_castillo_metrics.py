...
summary fig1C and fig4B use. The box is the conventional one, whiskers and caps included; at
n=7 read it as decoration rather than as statistics, since the 1.5 IQR rule calls a cell type
an outlier in 9 of these 48 positions. Fliers are off, so such a point is still drawn, just
as one of the coloured dots. Colour in this figure only ever means cell type. Every panel carries the model names on its x axis, so
